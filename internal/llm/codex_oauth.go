...
func NewCodexOAuthManager(cfg OAuthConfig) *CodexOAuthManager {
	tokenPath := cfg.TokenFile
	if tokenPath == "" {
		// Default to ~/.codex/auth.json like the official CLI
		homeDir, _ := os.UserHomeDir()
		tokenPath = filepath.Join(homeDir, ".codex", "auth.json")
	}
...
func (m *CodexOAuthManager) saveToken() error {
	...
	// Write with restricted permissions (like password file)
