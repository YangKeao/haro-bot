...
func New(db *gorm.DB, distance string) memory.VectorStore {
	return &store{db: db, table: "memories", distance: distance}
}

func (s *store) EnsureSchema(ctx context.Context, cfg config.MemoryConfig) error {
	if s == nil || s.db == nil {
...
func (s *store) ensureVectorDimensions(ctx context.Context, dims int) error {
	if dims <= 0 {
...
func (s *store) ensureVectorIndex(ctx context.Context, distance string) error {
...
func (s *store) ensureTiFlashReplica(ctx context.Context) error {
	log := logging.L().Named("memory_vector")
...
func (s *store) Insert(ctx context.Context, item memory.MemoryItem, vector []float32) (int64, error) {
	if s == nil || s.db == nil {
...
func (s *store) Update(ctx context.Context, item memory.MemoryItem, vector []float32) error {
	if s == nil || s.db == nil {
		return errors.New("vector store db required")
	}
	if item.ID == 0 {
...
func (s *store) Search(ctx context.Context, userID int64, sessionID *int64, vector []float32, limit int) ([]memory.MemoryItem, error) {
	if s == nil || s.db == nil {
	...
	items := make([]memory.MemoryItem, 0, len(rows))
...
func (r vectorRow) toItem() memory.MemoryItem {
