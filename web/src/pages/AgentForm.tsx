...
export default function AgentForm() {
  ...
  return <main className="page scroll-page agent-settings-page">
    <header className="editor-header settings-main-header">
      <div className="header-row"><Link to={id ? `/agents/${id}/sessions` : '/'} className="icon-button" aria-label="Back"><ArrowLeft /></Link><div><div className="eyebrow">{id ? 'Agent settings' : 'New agent'}</div><h1>{id ? `Edit ${existing.data?.name ?? ''}` : 'Create an agent'}</h1><p className="muted">Everything this agent needs, on one page.</p></div></div>
