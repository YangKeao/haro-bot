...
import { ArrowRight, Box, CircleAlert, Cpu, Database, HardDrive, MemoryStick, Plus } from 'lucide-react'
...
export default function Sandboxes() {
  ...
  return <main className="page scroll-page settings-page">
    <header className="page-header"><div><div className="eyebrow">Execution</div><h1>Sandboxes</h1><p className="muted">Persistent, isolated Kubernetes workspaces that one or more agents can share.</p></div><Link to="/sandboxes/new" className={`button primary ${disabled ? 'disabled-link' : ''}`} aria-disabled={disabled}><Plus size={16} /> New sandbox</Link></header>
    {disabled ? <div className="empty-state card"><CircleAlert /><h2>Sandbox support is disabled</h2><p>Enable the <code>[sandbox]</code> configuration and provide <code>HARO_SANDBOX_SECRET_KEY</code>, then restart Haro.</p></div> : query.isError ? <div className="form-error">{query.error instanceof APIError ? query.error.message : 'Could not load sandboxes.'}</div> :
      <div className="sandbox-grid">{query.isLoading ? [1, 2].map(i => <div className="sandbox-card skeleton" key={i} />) : query.data?.sandboxes.length ? query.data.sandboxes.map(item => <Link className="sandbox-card" key={item.id} to={`/sandboxes/${item.id}/edit`}>
        <div className="sandbox-card-head"><span className="sandbox-card-icon"><Box /></span><span className={`status-pill ${item.runtime_status === 'Ready' ? 'success' : item.last_error ? 'danger' : 'warning'}`}>{item.runtime_status || item.desired_state}</span></div>
        ...
        {item.pending_restart && <div className="pending-change">Changes waiting to be applied</div>}{item.last_error && <div className="sandbox-error">{item.last_error}</div>}<ArrowRight className="sandbox-arrow" />
