  ...
  return <main className="page scroll-page agent-settings-page">
    ...
    <div className="agent-settings-layout"><nav className="settings-outline"><a href="#identity"><Box /><span>Identity & image</span></a><a href="#resources"><Cpu /><span>Resources</span></a><a href="#agents"><Users /><span>Agents</span></a>{id && <><a href="#operations"><Play /><span>Operations</span></a><a href="#danger"><Trash2 /><span>Danger zone</span></a></>}</nav>
      <form className="settings-form" onSubmit={handleSubmit(values => save.mutate(values))}>
        ...
        {id && <section id="operations" className="form-section settings-section"><div className="section-heading"><h2>Lifecycle</h2><p>Pause deletes the Pod while keeping the workspace volume. Restart ends all running processes. In-container root filesystem changes are not persistent.</p></div><div className="sandbox-runtime-card"><div className="sandbox-runtime-status"><span className={`status-pill ${statusClass(profile?.runtime_status)}`}>{profile?.runtime_status || profile?.desired_state}</span><span className={`live-indicator ${stream}`}><Radio /> {stream === 'live' ? 'Live' : stream === 'connecting' ? 'Connecting' : 'Reconnecting'}</span></div><p>{profile?.runtime_details?.message || 'Waiting for runtime information.'}</p><div className="sandbox-runtime-meta"><span>Kubernetes resource: <code>{profile?.kubernetes_name}</code></span>{profile?.runtime_details?.pod && <><span>Pod: <code>{profile.runtime_details.pod.name}</code></span>{profile.runtime_details.pod.image && <span>Runtime image: <code title={profile.runtime_details.pod.image}>{profile.runtime_details.pod.image}</code></span>}<span>Started: {formatTimestamp(profile.runtime_details.pod.started_at || profile.runtime_details.pod.created_at)}</span><span>Restarts: {profile.runtime_details.pod.restart_count}</span></>}</div></div><div className="sandbox-lifecycle-actions"><div>{profile?.desired_state === 'Running' ? <button type="button" className="button secondary" onClick={() => action.mutate('pause')} disabled={lifecyclePending}><Pause /> Pause</button> : <button type="button" className="button secondary" onClick={() => action.mutate('start')} disabled={lifecyclePending}><Play /> Start</button>}<button type="button" className="button secondary" onClick={() => action.mutate('restart')} disabled={lifecyclePending || profile?.desired_state !== 'Running' || profile?.runtime_status !== 'Ready'}><RotateCw /> Restart</button></div>{profile?.runtime_status === 'Ready' ? <Link className="button primary" to={`/sandboxes/${id}/terminal`}><TerminalSquare /> Open terminal</Link> : <span className="button primary disabled-link" aria-disabled="true"><TerminalSquare /> Open terminal</span>}</div></section>}
