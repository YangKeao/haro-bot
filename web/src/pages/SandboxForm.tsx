...
export default function SandboxForm() {
  ...
  return <main className="page scroll-page agent-settings-page">
    <header className="editor-header settings-main-header"><div className="header-row"><Link to="/sandboxes" className="icon-button" aria-label="Back to sandboxes"><ArrowLeft /></Link><div><nav className="breadcrumbs" aria-label="Breadcrumb"><Link to="/sandboxes">Sandboxes</Link><span>/</span><span>{id ? profile?.name || 'Sandbox' : 'New'}</span></nav><div className="eyebrow">Execution workspace</div><h1>{id ? `Edit ${profile?.name || ''}` : 'Create a sandbox'}</h1><p className="muted">Runtime changes are staged until you explicitly apply them.</p></div></div><button className="button primary header-save" disabled={!canSave || !isDirty || save.isPending} onClick={handleSubmit(values => save.mutate(values))}>{save.isPending ? <LoaderCircle className="spin" /> : saved ? <Check /> : <Save />}{saved ? 'Saved' : id ? 'Save changes' : 'Create sandbox'}</button></header>
