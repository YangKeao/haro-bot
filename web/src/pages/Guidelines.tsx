...
export default function Guidelines() {
  ...
  return <main className="page scroll-page settings-page">
    <header className="page-header"><div><div className="eyebrow">Workspace behavior</div><h1>Global guideline</h1><p className="muted">Set the baseline behavior inherited by every agent.</p></div><button className="button primary" onClick={() => update.mutate()} disabled={!content.trim() || update.isPending}>{update.isPending ? <LoaderCircle className="spin" size={16} /> : saved ? <Check size={16} /> : <Save size={16} />}{saved ? 'Saved' : 'Save version'}</button></header>
